// Client–server wire-contract types for the chat domain.
// Only types that cross the network boundary live here.
...
//
// Client-side state aggregates (e.g. Room) live in chat/models/chat.model.ts.
// Must stay in sync with server-side chat/dtos/chat.dto.ts manually until a shared
...
export interface ChatUser {
  id: string; // stable client-generated UUID — persisted in AsyncStorage (mobile) / URL params (web)
  name: string;
  color: UserColor;
  joinedAt: Date;
}
