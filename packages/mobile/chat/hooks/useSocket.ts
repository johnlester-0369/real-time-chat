 ...
 * Identical to web's chat/hooks/useSocket.ts — React hooks work the same in RN.
 *
 * Delegates all socket lifecycle and protocol concerns to the service layer:
 *   - chat/lib/socket-client.ts  → owns the singleton Socket.IO connection
 *   - chat/services/socket.service.ts → owns all emit/subscribe calls
 *
 * @module chat/hooks/useSocket
 */

import { useEffect, useRef, useState, useCallback } from 'react';
import { socketService } from '@/chat/services/socket.service';
import type { Message, ChatUser, UserColor } from '@/chat/dtos/chat.dto';
...
export function useSocket(user?: UseSocketUser | null): UseSocketReturn {
  ...
  const sendMessage = useCallback(
    (text: string) => {
      ...
      // Silently drop if disconnected — the send button is disabled while offline
    },
    [isConnected],
  );
