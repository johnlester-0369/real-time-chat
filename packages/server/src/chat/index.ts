// Chat domain public entry point.
// This is the only file server.ts needs to import — it wires the Socket.IO
// lifecycle and delegates all room logic to the domain's internal service layer.
//
// Domain-based architecture: consumers outside the chat/ folder import from
// here, never from chat/lib/* or chat/services/* directly. This boundary keeps
// refactors within the domain from cascading into the app shell.
...
import { initSocketServer } from '@/chat/lib/socket-server.js';
import { roomService } from '@/chat/services/room.service.js';

// Public API surface mirrors the original export — server.ts requires no signature change.
// Internal implementation delegates entirely to lib/ (SocketServer lifecycle + CORS)
