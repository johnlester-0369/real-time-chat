 ...
 * Owned by the chat domain (chat/services/) — the service manages chat business logic,
 * not socket infrastructure, so it belongs to the domain, not a transport layer.
 *
 * Mirrors the web's chat/services/socket.service.ts pattern — consumers (chat/index.ts)
 ...
 *
 * @module chat/services/room.service
 */
...
} from '@/chat/dtos/chat.dto.js';
// Internal server models are not wire contracts — import from models/ not dtos/
import type {
  UserRecord,
  Room,
  PendingDisconnectEntry,
} from '@/chat/models/room.model.js';

// ============================================================================
// INTERNAL TYPES
// ============================================================================

// Typed aliases keep method signatures concise and consistent with chat/lib/socket-server.ts
...
class RoomService {
  ...
  private readonly userRegistry = new Map<
    string,
    { name: string; color: string; joinedAt: Date }
  >();
...
/**
 * Singleton service instance — import this directly in chat/index.ts.
